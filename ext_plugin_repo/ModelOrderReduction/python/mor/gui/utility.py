...
def openFileName(hdialog,filter="Sofa Scene (*.py *.pyscn)",display=None):
    ...
    global lastVisited
    if shortcut:
        QFileDialog.setSidebarUrls(QFileDialog(),shortcut)

    fileName = str(QFileDialog.getOpenFileName(None, hdialog,directory=lastVisited,filter=filter, options=QFileDialog.DontUseNativeDialog))
...
def openFilesNames(hdialog,filter="*.stl *.vtu *.vtk *.obj",display=None):
    ...
    tmp = QFileDialog.getOpenFileNames(None,hdialog,
...
def openDirName(hdialog,display=None):
    ...
    fileName = str(QFileDialog.getExistingDirectory(None,hdialog,
